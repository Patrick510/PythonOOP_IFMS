"""
4 - Teste as classes ZipReplace e ScaleZip para casos como: substituir palavras em arquivos de texto, redimensionar imagens com várias resoluções, verificar se arquivos fora do formato de imagem não são afetados por ScaleZip, e confirmar que todos os arquivos são reinseridos no arquivo ZIP final após a operação.
"""
...
class ProcessadorZip:
    ...
    def listar_arquivos_zip(self):
        with zipfile.ZipFile(self.nome_zip, 'r') as zip_file:
...
if __name__ == "__main__":
    ...
    # Verificar se todos os arquivos foram reinseridos no ZIP após a substituição
    assert set(listar_arquivos_diretorio(substituir_zip.diretorio_temporario)) == set(substituir_zip.listar_arquivos_zip())

    # Testar RedimensionarZip
    redimensionar_zip = RedimensionarZip("exemplo_redimensionar.zip")
    redimensionar_zip.processar_zip()

    # Verificar se todos os arquivos foram reinseridos no ZIP após o redimensionamento
    assert set(listar_arquivos_diretorio(redimensionar_zip.diretorio_temporario)) == set(redimensionar_zip.listar_arquivos_zip())